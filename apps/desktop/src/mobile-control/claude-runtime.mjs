...
function localRows() {
  return scanClaudeSessions({ limit: 500 }).map((session) => ({
...
function localMessages(sessionId) {
  const source = scanClaudeSessions({ limit: 1000 }).find((row) => row.sessionId === String(sessionId));
 ...
 */
export function createClaudeRuntime({ overlay, command, env, spawnProcess = spawnChild } = {}) {
  ...
  const sessionFor = (id) => sessions.get(String(id)) || localRows().find((row) => row.id === String(id));
  const listSessions = async ({ cwd } = {}) => localRows().filter((row) => !cwd || row.directory === cwd).map(remember);
  ...
  const sendMessage = async (sessionId, { text, attachments = [] } = {}) => {
    ...
    // 有图片时用 --input-format stream-json + stdin 原生传图片内容块；
    // 无图片时走原有文本 prompt 路径，stdin 忽略。
    const useStdin = images.length > 0;
    const materialized = useStdin ? { directory: "", files: [], cleanup() {} } : materializeImageAttachments(attachments);
    try {
      const context = textAttachments
        .map((item) => `\n\n<attachment name="${item.name}">\n${item.text}\n</attachment>`).join("");
      const exists = Boolean(scanClaudeSessions({ limit: 1000 }).find((item) => item.sessionId === sid));
      ...
      if (useStdin) {
        ...
        const contentBlocks = [];
        const promptText = `${String(text || "")}${context}`;
        ...
        if (materialized.directory) args.push("--add-dir", materialized.directory);
  ...
  return {
    ...
    listSessions,
    async readSession(sessionId) { const row = sessionFor(sessionId) || { id: String(sessionId), agentId: "claude-code", name: String(sessionId), state: "completed", updatedAt: null, model: "", directory: "", project: "", archived: false, capabilities: { ...CAPABILITIES } }; return { ...row, model: selectedModels.get(String(sessionId)) || row.model, messages: localMessages(sessionId) }; },
