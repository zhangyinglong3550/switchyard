...
| PDF | 尝试安全文本提取，提取失败时作为受控附件引用并提示 Agent 内容未提取 |
| DOCX/XLSX | 仅提取可公开的内嵌 XML 文本（不执行宏、不运行脚本）；提取失败则保留为受控附件引用 |
| ZIP | 不解压、不执行；仅作为受控附件引用，提示 Agent 需显式读取/解压到工作区 |
| 其他二进制 | 拒绝并返回可操作的类型提示 |

运行时收到受控二进制附件时，仅使用 Switchyard 创建的私有临时文件；临时文件在该轮结束后清理。对不能传递该类型的运行时，返回明确“不支持该附件类型”的错误，不能假装已传递。
