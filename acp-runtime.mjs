...
export function createAcpRuntime({
  ...
  client.subscribe((frame) => {
    if (frame.kind === "notification" && frame.method === "session/update") {
      ...
      // Switchyard 发送时已投影用户消息；Agent 回声的 user_message_chunk 若再
      // 发布到手机账本，会短暂出现两条相同气泡。
      if (event.runtimeEvent === "user_message_chunk") return;
  ...
  const sendMessage = async (sessionId, { text, messageId, attachments = [] } = {}) => {
    ...
    prompt.then((result) => {
      ...
      // 若已有流式内容，按完成处理，避免手机刷盘把回答冲掉。
      if (hadStream && Number(error?.code) === -32603) {
        const event = {
          sessionId: sid,
          type: "status",
          summary: "completed",
      ...
      const detail = error?.data && typeof error.data === "object"
        ...
        : "";
      const code = Number.isFinite(Number(error?.code)) ? `（${error.code}）` : "";
      const summary = `${error?.message || String(error)}${code}${detail ? ` ${detail}` : ""}`.trim();
      const failed = {
        sessionId: sid,
        type: "error",
        summary,
