...
export const SUPPORTED_CLIENTS = new Set(["codex", "claude-code", "claude-app", "hermes", "opencode", "grok", "deepseek-harness", "zcode", "generic-openai"]);
...
export const DEFAULT_CONFIG = {
  ...
  clients: {
    ...
    zcode: { enabled: true, allowedModels: ["*"], defaultModel: null },
...
function normalizeKnownProvider(provider) {
  ...
  const looksLikeAccountPool =
    ...
    ["xai_oauth", "antigravity_oauth", "codex_oauth", "workbuddy_oauth"].includes(withRouting.poolKind);
  if (looksLikeAccountPool) {
    ...
    if (withRouting.presetId === "workbuddy-account-pool") poolKind = "workbuddy_oauth";
    const defaults = {
      ...
      workbuddy_oauth: { baseUrl: "https://www.workbuddy.ai", apiFormat: "openai_chat" }
    };
