...
import { callOpenAIChat, callOpenAIResponses, callAnthropicMessages, readJsonResponse } from "./clients.mjs";
import { chatToResponses, responsesToChatResponse } from "../openai-adapter-out.mjs";
...
export async function dispatchChat(provider, upstreamModel, chatBody, opts = {}) {
  const ctx = { provider, model: { id: chatBody._modelId || upstreamModel, providerId: provider.id }, clientId: opts.clientId };
  const stream = Boolean(chatBody.stream);
  const outbound = applyOutbound({ ...chatBody, model: upstreamModel }, ctx);
  const apiFormat = provider.apiFormat || "openai_chat";

  if (apiFormat === "openai_chat") {
    const upstream = await callOpenAIChat(provider, outbound, opts);
  ...
  if (apiFormat === "openai_responses") {
    ...
    const upstream = await callOpenAIResponses(provider, responsesBody, opts);
  ...
  if (apiFormat === "anthropic_messages") {
    ...
    const upstream = await callAnthropicMessages(provider, anthBody, opts);
